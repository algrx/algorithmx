...
import { AnyAttrDef } from '../attributes/definitions';
...
export const definition: AnyAttrDef<AnimationFull<ICanvasAttr>> = attrAnim.createFullDef(
    attrCanvas.definition,
    attrAnim.definition
);

type InputAnimFull<T> = MapEndpoints<PartialAttr<T>, PartialAttr<IAnimation>>;
const addEndpoints = <T extends Attr>(
    anim: InputAnimAttr<T> | null,
    attr: PartialAttr<T>,
    def: AnyAttrDef<T>,
    curAnim: PartialAttr<IAnimation> = {}
): InputAnimFull<T> => {
    if (attrUtils.isDefPrimitive(def)) return (anim !== null ? anim : curAnim) as InputAnimFull<T>;
        ...
        return (attrUtils.reduceChanges(attr, def, (k, v, d) => {
            ...
            const newCurAnim =
                anim !== null && anim['**'] !== undefined
                    ? attrUtils.merge(curAnim, anim['**'], attrAnim.definition)
                    : curAnim;
...
const getRelevantChanges = <T extends Attr>(
...
export const process = (
    animation: InputCanvasAnimAttr,
    prevState: ICanvasAttr | undefined,
    changes: PartialAttr<ICanvasAttr>,
    changesForced: PartialAttr<ICanvasAttr>
    ...
    const animDefaultsExtra = attrUtils.fillLookupEntries(
        attrCanvas.animationDefaults,
        animDefaultsRegular,
        definition
    );
