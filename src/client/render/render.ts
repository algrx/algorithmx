...
export type RenderAttrFn<T extends Attr> = (
...
export const transAnimate = (trans: D3Transition, animation: IAnimation): D3Transition => {
    if (isAnimationImmediate(animation)) return trans.duration(0);
        ...
        return trans
            .duration(parseTime(animation.duration))
            .ease(renderUtils.easeFn(animation.ease));
};
...
export const render = <T extends Attr>(
    selection: D3Selection,
    renderData: RenderEndpoint<T>,
    renderFn: RenderFn<T>
): D3SelTrans => {
...
export const renderHighlight = <T extends Attr>(
    selection: D3Selection,
    renderData: RenderEndpoint<T>,
    renderStartFn: RenderFn<T>,
    renderEndFn: RenderFn<T>
): D3SelTrans => {
    const initTrans = renderStartFn(
        animate(selection, renderData.name, renderData.animation),
        renderData.highlight
    );

    const linger = renderData.animation ? parseTime(renderData.animation.linger) : 0;
    const newTrans = newTransition(initTrans, (t) =>
        transAnimate(t.delay(linger), renderData.animation)
    );
    return renderEndFn(newTrans, renderData.attr);
};

export type RenderLookupFn<T> = (k: string, renderData: RenderAttr<T>) => void;
export const renderLookup = <T extends Attr>(
    renderData: RenderAttr<AttrLookup<T>>,
    renderFn: RenderLookupFn<T>
): void => {
