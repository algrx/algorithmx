...
export const selectOverlay = (
    edgeSel: D3Selection,
    renderData: RenderAttr<IEdgeAttr>
): D3Selection => {
    edgeSel.select('.edge-path-overlay').remove();
    const overlay = edgeSel
        .append('path')
    ...
    if (renderData.attr.directed)
        overlay.attr('marker-end', `url(#${renderMarker.getFullId(edgeSel, 'target')}`);
    return overlay;
};
...
const highlightTraverse = (
    pathSel: D3Selection,
    overlaySel: D3Selection,
    renderData: RenderAttr<IEdgeAttr>
): void => {
    const colorData = getEntry(renderData, 'color');
    ...
    overlaySel
        .attr('stroke', renderUtils.parseColor(colorData.highlight))
...
export const renderTraverse = (
    ...
    if (colorData.highlight !== undefined) highlightTraverse(pathSel, overlaySel, renderData);
        ...
        renderFns
            .transition(pathSel, colorData.name, (t) => t.delay(animDuration).duration(endDuration))
            .attr('stroke', renderUtils.parseColor(colorData.attr));

        const removeTrans = renderFns
...
export const renderColor = (
    ...
    if (doTraverse) renderTraverse(pathSel, renderData, overlaySelector);
        ...
        renderElement.renderSvgAttr(pathSel, 'stroke', (v) => renderUtils.parseColor(v), colorData);
