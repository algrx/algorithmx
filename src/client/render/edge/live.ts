...
export const getLoopAngle = (canvasSel: D3Selection, layoutState: ILayoutState,
                             canvasAttr: AttrEval<ICanvasAttr>, nodeId: string): number => {
  ...
  const maxDifIndex = angleDiffs.reduce((maxIndex, a, i) => a > angleDiffs[maxIndex] ? i : maxIndex, 0)
...
export const getLiveEdgeData = (canvasSel: D3Selection, layoutState: ILayoutState,
  ...
  const angle = edgeAttr.source === edgeAttr.target ? getLoopAngle(canvasSel, layoutState, canvasAttr, edgeAttr.source)
    : math.restrictAngle(Math.atan2(targetData.pos[1] - sourceData.pos[1], targetData.pos[0] - sourceData.pos[0]))
