...
export const select = (edgeSel: D3Selection, markerId: string): D3Selection => {
  const defsSel = renderUtils.selectOrAdd(edgeSel, 'defs', s => s.insert('svg:defs', ':first-child'))
  const fullId = getFullId(edgeSel, markerId)
  return renderUtils.selectOrAdd(defsSel, `#${fullId}`, s => {
    const marker = s.append('svg:marker').attr('id', fullId).classed(`marker-${markerId}`, true)
    marker.append('path')
    return marker
  })
}
