import { LabelAttrSpec, Align, ALIGN_ANGLES } from '../../attributes/definitions/label';
...
const renderPos = (selection: D3SelTrans, posData: LabelPosData): D3SelTrans => {
    ...
    return selection.attr(
        'transform',
        `translate(${posData.x + polarX},${-(posData.y + polarY)})${rotateStr}`
    );
};

const isAlignTop = (a: Align): boolean =>
    a === 'top-left' || a === 'top-middle' || a === 'top-right';
const isAlignBottom = (a: Align): boolean =>
    a === 'bottom-left' || a === 'bottom-middle' || a === 'bottom-right';

export const renderAlign = (
    selection: D3Selection,
    renderData: RenderAttr<LabelAttrSpec['align']>
): void => {
    renderElement.renderSvgAttr(
        selection,
        'y',
        (v) => (isAlignTop(v) ? '0.75em' : isAlignBottom(v) ? '0em' : '0.25em'),
    ...
    renderElement.renderSvgAttr(
        ...
        'text-anchor',
        (v) =>
            v === 'top-left' || v === 'middle-left' || v === 'bottom-left'
                ? 'start'
                : v === 'top-right' || v === 'middle-right' || v === 'bottom-right'
                ? 'end'
                : 'middle',
        { ...renderData, name: renderData.name + '-x' }
    );
};
...
export const preprocessAlign = (
    labelData: RenderAttr<LabelAttrSpec>
): RenderAttr<LabelAttrSpec['align']> => {
    const align = labelData.attr.align;
    const changedRadialAlign =
        labelData.changes &&
        (labelData.changes.align !== undefined ||
            labelData.changes.angle !== undefined ||
            labelData.changes.rotate !== undefined);

    return {
        ...getEntry(labelData, 'align'),
        attr:
            align === 'radial'
                ? attrLabel.alignFromAngle(
                      math.angleToRad(labelData.attr.angle),
                      labelData.attr.rotate
                  )
                : align,
        changes:
            align === 'radial' && changedRadialAlign
                ? align
                : labelData.changes
                ? labelData.changes.align
                : undefined,
    };
};

export const render: renderFns.RenderAttrFn<LabelAttrSpec> = (selection, renderData) => {
    ...
    const combinedTextAlign = renderProcess.combine({
        text: getEntry(renderData, 'text'),
        align: alignData,
    });
    ...
    renderFns.render(textSel, combinedPos, (sel, posData) => {
        return renderPos(sel, { ...posData, angle: math.angleToRad(posData.angle) });
    });
    ...
    renderElement.renderSvgAttr(
        textSel,
        'fill',
        (v) => renderUtils.parseColor(v),
        getEntry(renderData, 'color')
    );
    ...
    renderElement.renderSvgAttrMixin(textSel, renderData);
};
