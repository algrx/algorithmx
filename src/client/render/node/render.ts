...
const renderSize = (
    ...
    sizeData: RenderAttr<INodeAttr['size']>,
    shape: Shape
): void => {
    ...
    switch (shape) {
        ...
        case 'rect':
            ...
            renderElement.renderSvgAttr(selection, 'x', (v) => -v, {
                ...width,
                name: width.name + '-pos',
            });
            renderElement.renderSvgAttr(selection, 'y', (v) => -v, {
                ...height,
                name: height.name + '-pos',
            });
            break;
        case 'ellipse':
...
export const preprocess = (renderData: RenderAttr<INodeAttr>): RenderAttr<INodeAttr> => {
    const shapeUpdateKeys = nodeAttr.definition.keyOrder.filter(
        (k) => k !== 'pos' && k !== 'visible' && k !== 'labels'
    );
    return renderProcess.hasChanged(getEntry(renderData, 'shape'))
        ? renderProcess.markKeysForUpdate(renderData, shapeUpdateKeys)
        : renderData;
};

export const renderVisible: renderFns.RenderAttrFn<INodeAttr['visible']> = (
    selection,
    renderData
) => {
...
export const render: renderFns.RenderAttrFn<INodeAttr> = (selection, renderDataInit) => {
    ...
    renderFns.render(selection, getEntry(renderData, 'shape'), (s, shape) =>
        renderShape(selection, shape)
    );
