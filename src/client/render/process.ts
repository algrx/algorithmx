...
export const combine = <T extends object>(
    renderDict: { readonly [k in keyof T]: RenderEndpoint<T[k]> }
): RenderEndpoint<T> => {
    const hasHighlight = !utils.isDictEmpty(
        utils.filterDict(renderDict, (k, v) => v.highlight !== undefined)
    );
    const highlight = !hasHighlight
        ? undefined
        : (utils.mapDict(renderDict, (k, v) =>
              v.highlight !== undefined ? v.highlight : v.attr
          ) as RenderEndpoint<T>['highlight']);

    const changesDict = utils.filterDict(
        utils.mapDict(renderDict, (k, v) => v.changes),
        (k, v) => v !== undefined
    );
    const changes = utils.isDictEmpty(changesDict)
        ? undefined
        : (changesDict as RenderEndpoint<T>['changes']);

    const animation = Object.entries(renderDict).reduce(
        // result = [animation, lockedIn]
        (
            result: [IAnimation | undefined, boolean],
            [k, renderData]: [string, RenderEndpoint<T[keyof T]>],
            i
        ) => {
            if (
                result[1] === false &&
                renderData.changes !== undefined &&
...
export const markForUpdate = <T extends AttrRecord>(renderAttr: RenderAttr<T>): RenderAttr<T> => {
    return markKeysForUpdate(
        renderAttr,
        (Object.keys(renderAttr.attr) as unknown) as ReadonlyArray<keyof T>
    );
};
export const markKeysForUpdate = <T extends AttrRecord>(
    renderAttr: RenderAttr<T>,
    keys: ReadonlyArray<keyof T>
): RenderAttr<T> => {
    const newChanges = keys.reduce(
        (result, k) => ({ ...result, [k]: renderAttr.attr[k as string] }),
        {}
    );
    return {
        ...renderAttr,
