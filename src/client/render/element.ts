...
import { ElementAttrSpec, ISvgMixinAttr } from '../attributes/definitions/element';
...
export const renderVisibleLookup = <T extends ElementAttrSpec>(
    renderData: RenderAttr<AttrLookup<T>>,
    renderFn: renderFns.RenderLookupFn<T>
): void => {
...
export const renderSvgAttr = <T extends Attr>(
    selection: D3Selection,
    key: string,
    valueFn: (v: AttrEval<T>) => Primitive,
    attr: RenderEndpoint<T>
): D3SelTrans => {
...
export const renderSvgAttrMixin: renderFns.RenderAttrFn<ISvgMixinAttr> = (
    selection,
    renderData
) => {
...
export const renderVisible: renderFns.RenderAttrFn<ElementAttrSpec['visible']> = (
    selection,
    renderData
) => {
...
export const renderRemove: renderFns.RenderAttrFn<ElementAttrSpec['visible']> = (
    selection,
    renderData
) => {
    renderFns.onChanged(selection, renderData, (sel, visible) => {
        if (visible.attr === false) {
            if (renderFns.isAnimationImmediate(visible.animation)) sel.remove();
                ...
                renderFns
                    .transition(sel, 'remove', (t) =>
                        t.delay(renderFns.parseTime(visible.animation.duration))
                    )
                    .remove();
        }
    });
...
export const preprocess = <T extends ElementAttrSpec>(renderData: RenderAttr<T>): RenderAttr<T> => {
...
export const renderElementRemove = <T extends ElementAttrSpec>(
    selection: D3Selection,
    renderData: RenderAttr<T>
) => {
    const newVisible = { attr: { visible: false }, changes: { visible: false } } as RenderAttr<
        PartialAttr<ElementAttrSpec>
    >;
...
export const renderElement = <T extends ElementAttrSpec>(
    selector: () => D3Selection,
    renderData: RenderAttr<T>,
...
export const renderElementLookup = <T extends ElementAttrSpec>(
