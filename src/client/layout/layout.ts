...
export const update = (
    ...
    attr: AttrEval<ICanvasAttr>,
    changes: AttrEvalPartial<ICanvasAttr>
): ILayoutState => {
    const didUpdateNodes =
        changes.nodes &&
        Object.entries(changes.nodes).findIndex(
            ([k, v]) => v === null || layoutNode.didUpdateLayout(attr.nodes[k], v)
        ) >= 0;
    const didUpdateEdges =
        changes.edges &&
        Object.values(changes.edges).findIndex(
            (v) => v === null || layoutEdge.didUpdateLayout(v)
        ) >= 0;
    ...
    const newNodes = didUpdateNodes
        ? layoutNode.mergeLookup(layoutState.nodes, attr.nodes, changes.nodes)
        : layoutState.nodes;
    ...
    const adjacency =
        didUpdateNodes === undefined && didUpdateEdges === undefined
            ? layoutState.adjacentNodes
            : layoutCanvas.calcAdjacency(attr.nodes, attr.edges);
