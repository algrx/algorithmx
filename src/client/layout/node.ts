...
const fromAttrChanges = (
    attr: AttrEval<INodeAttr>,
    changes: AttrEvalPartial<INodeAttr>
): Partial<NodeLayout> => {
    const height = attr.shape === 'circle' ? attr.size.width * 2 : attr.size.height * 2;
    const changedHeight =
        changes.size &&
        (attr.shape === 'circle'
            ? changes.size.width !== undefined
...
export const mergeLookup = (
    nodes: Lookup<NodeLayout>,
    attr: AttrEval<AttrLookup<INodeAttr>>,
    changes: AttrEvalPartial<AttrLookup<INodeAttr>>
): Lookup<NodeLayout> => {
    ...
    return Object.keys(nodes)
        .concat(Object.keys(changes))
        .reduce(
            (result, k) =>
                nodeChanges[k] === null
                    ? result // remove node
                    : nodes[k] === undefined
                    ? { ...result, [k]: nodeChanges[k] as NodeLayout } // add node
                    : nodeChanges[k] === undefined
                    ? { ...result, [k]: nodes[k] } // unchanged
