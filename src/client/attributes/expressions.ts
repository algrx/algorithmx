...
import { AttrType, AnyAttrDef, BaseAttrType, PrimitiveAttrDef } from './definitions';
...
const parseSingleTerm = (term: string): ITerm | Error => {
    const reverseIndex = term
        .split('')
        .findIndex((c, i) => utils.isNumStr(term.substring(0, term.length - i)));
    const varIndex =
        reverseIndex === -1
            ? term[0] === '-' || term[0] === '+'
                ? 1
                : 0
            : term.length - reverseIndex;
...
export const parseExprObj = (
    value: object,
    vars: ReadonlyArray<VarSymbol>,
    formatted?: string
): INumExpr | Error => {
    ...
    if (
        (numExpr.m !== undefined && typeof numExpr.m !== 'number') ||
    ...
    if (!vars.includes(numExpr.x))
        return new Error(
            `expression '${formattedExpr}' uses invalid variable '${numExpr.x}'` +
...
export const isExpression = (attr: AttrPrimitive, def: BaseAttrType): boolean => {
