...
export type AttrEntry2<T extends Attr> = T extends AttrEvalPartial<infer E>
    ? AttrEvalPartial<AttrEntry<E>>
    : never;

export type AttrEntry<T extends Attr> = T extends AttrPrimitive
    ? T
    : T extends AttrLookup<infer E2>
    ? E2
    : T extends AttrArray<infer E1>
    ? E1
...
export type AttrPrimEvaluated<T extends AttrPrimitive> = T extends AttrNum
    ? number
    : T extends AttrString
    ? T & string
    : T extends AttrBool
    ? boolean
    : never;
...
export type AttrEval<T extends Attr> = T extends AttrPrimitive
    ? AttrPrimEvaluated<T>
    : T extends AttrArray<infer A>
    ? T // should be: AttrArray<AttrEval<A>>
    : { [k in keyof T]: AttrEval<T[k]> };

export type PartialAttr<T extends Attr> = T extends AttrPrimitive
    ? T
    : T extends AttrArray<infer A>
    ? T // should be: AttrArray<PartialAttr<A>>
...
type StrNumTuple<K1 extends string, K2 extends string, S extends string> = {
    [k in K1 | K2]: S | AttrNum;
};
type InputStrNumTuple<S extends string> = S | [S, InputNum];

export type InputAttr<T extends Attr> = T extends AttrNum
    ? InputNum
    : T extends AttrPrimitive
    ? T
    ...
    T extends NumTuple<infer NK2, infer NK2>
    ? InputNumTuple | InputAttrRec<T>
    : T extends StrNumTuple<infer SNK1, infer SNK2, infer S>
    ? InputStrNumTuple<S> | InputAttrRec<T>
    : T extends AttrRecord
    ? InputAttrRec<T>
    : never;
