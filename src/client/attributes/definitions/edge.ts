...
export interface IEdgeAttr extends ElementAttrSpec, ISvgMixinAttr {
...
export const definition = attrDef.extendRecordDef<IEdgeAttr, ElementAttrSpec>(
    {
        entries: {
...
export const initChildren = (
    prevAttr: IEdgeAttr,
    changes: PartialAttr<IEdgeAttr>
): PartialAttr<IEdgeAttr> => {
...
const incrementMatrix = (matrix: EdgeAdjMatrix, source: string, target: string): EdgeAdjMatrix => {
    ...
    const sourceLookup = {
        ...sourceAdj,
        [target]: sourceAdj[target] === undefined ? 1 : sourceAdj[target] + 1,
    };
    const targetLookup = {
        ...targetAdj,
        [source]: targetAdj[source] === undefined ? 1 : targetAdj[source] + 1,
    };
...
export const initLookup = (
    prevEdges: AttrLookup<IEdgeAttr>,
    changes: PartialAttr<AttrLookup<IEdgeAttr>>
    ...
    const edgeInitData = Object.entries(newEdges).reduce(
        (result, [k, edge]) => {
