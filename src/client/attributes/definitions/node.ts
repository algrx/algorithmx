...
import { AttrType, RecordAttrDef, PrimitiveAttrDef } from '../definitions';
...
interface NodeAttrDef extends RecordAttrDef {
    readonly entries: {
        readonly shape: PrimitiveAttrDef & {
            readonly validValues: Shape;
        };
    };
}

export interface INodeAttr extends ElementAttrSpec, ISvgMixinAttr {
    readonly labels: AttrLookup<LabelAttrSpec>;
...
export const definition = attrDef.extendRecordDef<INodeAttr, ElementAttrSpec>(
    {
        entries: {
            labels: {
...
export const defaults: INodeAttr = {
    ...
    labels: {} as AttrLookup<LabelAttrSpec>,
    shape: 'circle',
...
export const init = (name: string, index: number): INodeAttr => {
    const defaultLabel: LabelAttrSpec = {
        ...attrLabel.init(name),
        align: 'middle',
        pos: { x: 0, y: -1 },
        radius: 0,
    ...
    return {
        ...defaults,
        labels: ({ [VALUE_LABEL]: defaultLabel } as unknown) as AttrLookup<LabelAttrSpec>,
...
export const initChildren = (
    prevAttr: INodeAttr,
    changes: PartialAttr<INodeAttr>
): PartialAttr<INodeAttr> => {
    ...
    const newOtherLabelChanges = utils.filterDict(
        newLabels,
        (k) => newRadialLabelChanges[k] === undefined
    );

    const newRadialLabels = utils.mapDict(
        newRadialLabelChanges,
        (k, v, i): PartialAttr<LabelAttrSpec> => {
            const index = Object.keys(prevRadialLabels).length + i;
            const defaultAngle =
                (Math.PI * 3) / 4 -
                (Math.PI / 2) * (index % 4) -
                Math.floor(index / 4) * (Math.PI / 4);
            return {
...
export const evaluate = (
    evaluated: AttrEvalPartial<INodeAttr>,
    expr: PartialAttr<INodeAttr>,
    changes: PartialAttr<INodeAttr>
): AttrEvalPartial<INodeAttr> => {
    const evalChanges = attrExpr.getEvaluatedChanges(expr, getVariables(evaluated), definition);
    const newEval = attrUtils.merge(evaluated, evalChanges || {}, definition) as AttrEvalPartial<
        INodeAttr
    >;

    const evalLabelChanges = attrUtils.reduceChanges<INodeAttr['labels']>(
        expr.labels || {},
        definition.entries.labels,
        (k, labelExpr) =>
            newEval.labels && newEval.labels[k]
                ? attrExpr.getEvaluatedChanges(
                      labelExpr,
                      getLabelVariables(newEval, newEval.labels[k]),
                      attrLabel.definition
                  )
                : undefined
    );

    const evalChildChanges: PartialAttr<INodeAttr> = { labels: evalLabelChanges || {} };
    return attrUtils.merge(evalChanges || {}, evalChildChanges, definition) as AttrEvalPartial<
        INodeAttr
    >;
};
...
export const getVariables = (attr: AttrEvalPartial<INodeAttr>): attrExpr.VarLookup => {
    ...
    return {
        ...
        ...(attr.shape !== undefined && attr.shape === 'circle' && hasWidth
            ? { [EnumVarSymbol.Height]: attr.size.width }
            : hasHeight
            ? { [EnumVarSymbol.Height]: attr.size.height }
            : {}),
    };
};

export const getLabelVariables = (
    node: AttrEvalPartial<INodeAttr>,
    label: AttrEvalPartial<LabelAttrSpec>
): attrExpr.VarLookup => {
    if (
        node.size &&
        node.size.width !== undefined &&
        node.size.height !== undefined &&
        label.angle !== undefined
    ) {
        const radius = radiusAtAngle(
            math.angleToRad(label.angle),
            node.size.width,
            node.size.height,
            node.shape
        );
