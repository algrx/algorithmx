...
import { RecordAttrType } from '../attr-type';
...
export interface ICanvasAttr extends ElementAttrSpec, ISvgMixinAttr {
            ...
            edgelengths: {
                ...
                entries: {
                    type: {
                        ...
                        validValues: utils.enumValues(EnumEdgeLengthType),
                    },
...
const labelDefaults: PartialAttr<LabelAttrSpec> = {
    align: 'middle',
...
export const init = (canvasSize: [number, number]): ICanvasAttr => {
    return { ...defaults, size: { width: canvasSize[0], height: canvasSize[1] } };
};

export const initChildren = (
    prevAttr: ICanvasAttr,
    changes: PartialAttr<ICanvasAttr>
): PartialAttr<ICanvasAttr> => {
    const newNodes = utils.mapDict(
        attrUtils.newLookupEntries(prevAttr.nodes, changes.nodes),
        (k, node, i) => {
    ...
    const newLabels = utils.mapDict(
        attrUtils.newLookupEntries(prevAttr.labels, changes.labels),
        (k) => ({ ...attrLabel.init(k as string), ...labelDefaults })
    );

    const nodeChildrenInit = attrUtils.reduceChanges<ICanvasAttr['nodes']>(
        changes.nodes || {},
        definition.entries.nodes,
        (k, v) => (prevAttr.nodes[k] ? attrNode.initChildren(prevAttr.nodes[k], v) : {})
    );

    const edgeChildrenInit = attrUtils.reduceChanges<ICanvasAttr['edges']>(
        changes.edges || {},
        definition.entries.edges,
        (k, v) => (prevAttr.edges[k] ? attrEdge.initChildren(prevAttr.edges[k], v) : {})
    );
...
export const evaluate = (
    evaluated: AttrEvalPartial<ICanvasAttr>,
    expr: PartialAttr<ICanvasAttr>,
    changes: PartialAttr<ICanvasAttr>
    ...
    const evalNodeChanges = attrUtils.reduceChanges<ICanvasAttr['nodes']>(
        newChanges.nodes || {},
        definition.entries.nodes,
        (k, node) =>
...
export const removeInvalidEdges = (
    prevAttr: ICanvasAttr | undefined,
    changes: PartialAttr<ICanvasAttr>
): PartialAttr<ICanvasAttr> => {
