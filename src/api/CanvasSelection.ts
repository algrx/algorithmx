...
const receiveHandler = (event: events.ReceiveEvent, listeners: selection.SelListeners): void => {
    if (event.type === events.EnumReceiveType.broadcast)
        ...
        selection.triggerListener(
            listeners,
            `${event.data.entered ? 'hoverin' : 'hoverout'}-node-${event.data.id}`
        );
    else if (event.type === events.EnumReceiveType.error) console.error(event.data.message);
};

const builder: ClassBuilder<CanvasSelection, ISelContext<InputCanvasAttr>> = (
    context,
    self,
    construct
) =>
    utils.inherit<CanvasSelection, Selection<InputCanvasAttr>>(
        {
            ...
            edges: (edges) => {
                const ids = edges.map(([source, target, edgeId]) => {
                    ...
                    return `${orderedNodes[0]}-${orderedNodes[1]}${
                        edgeId !== undefined ? '-' + String(edgeId) : ''
                    }`;
                });
            ...
            edgelengths: (lengthInfo) => {
                context.client.dispatch(
                    utils.attrEvent(context, lengthInfo, (d) => ({ edgelengths: d }))
                );
                return self();
            },
            ...
            zoomkey: (required) => {
                context.client.dispatch(
                    utils.attrEvent(context, required, (d) => ({ zoomkey: d }))
                );
                return self();
            },
