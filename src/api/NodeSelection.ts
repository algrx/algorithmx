...
const builder: ClassBuilder<NodeSelection, ISelContext<InputNodeAttr>> = (
    context,
    self,
    construct
) =>
    utils.inherit<NodeSelection, Selection<InputNodeAttr>>(
        {
            ...
            draggable: (draggable) => {
                context.client.dispatch(
                    utils.attrEvent(context, draggable, (d) => ({ draggable: d }))
                );
                return self();
            },
            click: (onClick) => {
                ...
                context.ids.forEach((id, i) => {
                    selection.addListener(context.listeners, `click-node-${id}`, () =>
                        onClick(context.data[i], i)
                    );
                });
                return self();
            },
            hoverin: (onHoverin) => {
                ...
                context.ids.forEach((id, i) => {
                    selection.addListener(context.listeners, `hoverin-node-${id}`, () =>
                        onHoverin(context.data[i], i)
                    );
                });
                return self();
            },
            hoverout: (onHoverout) => {
                context.client.dispatch(utils.attrEvent(context, true, (d) => ({ hover: d })));
                context.ids.forEach((id, i) => {
                    selection.addListener(context.listeners, `hoverout-node-${id}`, () =>
                        onHoverout(context.data[i], i)
                    );
                });
                return self();
            },
