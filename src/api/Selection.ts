...
export const defaultContext = (): Omit<
    ISelContext<IElementAttr>,
    'client' | 'initattr' | 'data'
> => ({
    name: '',
    ids: [],
    listeners: {},
...
export const builder: ClassBuilder<Selection<InputElementAttr>, ISelContext<InputElementAttr>> = (
    ...
    eventQ: (queue = 'default') =>
        construct({ ...context, queue: queue === null ? null : String(queue) }),
    ...
    highlight: (seconds) =>
        construct({
            ...
            animation:
                seconds !== undefined
                    ? utils.updateAnimation(context, seconds, (d) => ({ linger: d }))
                    : context.animation,
        }),
    ...
    data: (data) =>
        construct({
            ...
            data:
                data === null
                    ? null
                    : context.ids.map((id, i) =>
                          typeof data === 'function'
                              ? data(context.data === null ? null : context.data[i], i)
                              : Array.isArray(data)
                              ? data[i]
                              : data
                      ),
...
export const svgMixinAttrBuilder = <
    T extends InputSvgMixinAttr & InputElementAttr,
    S extends Selection<T>
>(
    context: ISelContext<T>,
    self: () => S
) => ({
    svgattr: (key: string, value: ElementArg<string | number | null>) => {
        context.client.dispatch(
            utils.attrEvent(context, value, (d) => ({ svgattr: { [key]: d } } as T))
        );
        return self();
    },
});
