...
const builder: ClassBuilder<EdgeSelection, ISelContext<InputEdgeAttr>> = (
    context,
    self,
    construct
) =>
    utils.inherit<EdgeSelection, Selection<InputEdgeAttr>>(
        {
            traverse: (source = (e, i) => context.initattr[i].source) => {
                return construct({
                    ...context,
                    animation: utils.updateAnimation(context, source, (d) => ({
                        type: 'traverse',
                        data: { source: String(d) },
                    })),
                });
            },
            ...
            directed: (directed) => {
                context.client.dispatch(
                    utils.attrEvent(context, directed, (d) => ({ directed: d }))
                );
                return self();
            },
            length: (length) => {
            ...
            thickness: (thickness) => {
                context.client.dispatch(
                    utils.attrEvent(context, thickness, (d) => ({ thickness: d }))
                );
                return self();
            },
            color: (color) => {
