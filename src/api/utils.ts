import {
    ...
    InputCanvasAttr,
    InputCanvasAnimAttr,
} from '../client/attributes/definitions/types';
...
const getAttrEntry = <T extends InputElementAttr, A>(
    sel: ISelContext<T>,
    arg: ElementArg<A>,
    attr: AttrFromArg<T, A>,
    index: number
): T => {
    if (typeof attr === 'function') {
        const evalArg =
            typeof arg === 'function' ? (arg as ElementFn<A>)(sel.data[index], index) : arg;
...
const createParentAttr = <T extends InputElementAttr, P extends InputElementAttr, A>(
    ...
    if (typeof attr === 'function' && sel.data === null) {
        return ((a: A) => ({
            [sel.name]: sel.ids.reduce(
                (result, id) => ({ ...result, [id]: (attr as AttrFromArgFn<T, A>)(a) }),
                {}
            ),
        ...
        return {
            [sel.name]: sel.ids.reduce(
                (result, id, i) => ({ ...result, [id]: getAttrEntry(sel, arg, attr, i) }),
                {}
            ),
        } as P;
    }
...
export const queueEvent = <T extends InputElementAttr>(
    sel: ISelContext<T>,
    type: events.IDispatchQueueUpdate['type'],
    queue: unknown | null
): events.IDispatchQueueUpdate => {
...
export const mergeDictRec = (a: object, b: object): object => {
    const merged = Object.keys(a).reduce(
        (res, k) => ({
            ...res,
            [k]:
                isDict(a[k]) && isDict(b[k])
                    ? mergeDictRec(a[k], b[k])
                    : b[k] !== undefined
                    ? b[k]
                    : a[k],
        }),
...
export const updateAnimation = <T extends InputElementAttr, A>(
    sel: ISelContext<T>,
    arg: ElementArg<A>,
    attr: (a: A) => Partial<IAnimation>
): InputCanvasAnimAttr => {
    if (
        (isDictEmpty(sel.animation) ||
            (Object.keys(sel.animation).length === 1 && sel.animation['**'] !== undefined)) &&
        typeof arg !== 'function'
    )
        // optimization to minimize the amount of transmitted data
        return mergeDictRec(sel.animation, { '**': attr(arg) });
    else {
        const animationAttr = createFullAttr(
            sel,
            arg,
            (a: A) => (({ '**': attr(a) } as unknown) as T)
        ) as InputCanvasAnimAttr;
