...
it('Queue | Pause in series', () => {
    ...
    return new Promise((resolve) => {
        ...
        setTimeout(
            () => expect(getNodeColors()).to.eql([utils.RED, utils.RED, utils.RED]),
            DELAY - DELTA
        );
        setTimeout(
            () => expect(getNodeColors()).to.eql([utils.GREEN, utils.RED, utils.RED]),
            2 * DELAY - DELTA
        );
        setTimeout(
            () => expect(getNodeColors()).to.eql([utils.GREEN, utils.GREEN, utils.RED]),
            3 * DELAY - DELTA
        );
        setTimeout(
            () => expect(getNodeColors()).to.eql([utils.GREEN, utils.GREEN, utils.GREEN]),
            4 * DELAY - DELTA
        );
